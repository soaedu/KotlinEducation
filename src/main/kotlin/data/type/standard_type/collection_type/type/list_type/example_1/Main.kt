...
fun main() {
    // Read-only instances
    // #1
    // Use 'List()' to crate a read-only list.
    val ints_1 = List(10) { it + 1 }
    // #2
    // Use 'listOf()' to crate a read-only list.
    val ints_2: List<Int> = listOf(20, 29, 40, 10)
    // #3
    // Use 'emptyList()' to crate a read-only list.
    val emptyList: List<String> = emptyList()
    // #4
    // Use 'listOfNotNull()' to crate a read-only list.
    val notNulls: List<String> = listOfNotNull(null, "a", "b", "c")

    // Mutable instances
    // #5
    // Use 'arrayListOf()' to crate a mutable list.
    val doubles: ArrayList<Double> = arrayListOf(84.88, 100.25, 999.99)
    // #6
    // Use 'mutableListOf' to crate a mutable list.
